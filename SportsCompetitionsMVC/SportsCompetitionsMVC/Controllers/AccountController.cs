...
using SportsCompetitionsMVC.DbOperations;
using SportsCompetitionsMVC.Models;
using System;
using System.IO;
using System.Linq;

namespace SportsCompetitions.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var extension = System.IO.Path.GetExtension(model.Avatar.FileName);

                    editedUser.Informations.Avatar = "/Content/Upload/" + filename + extension;

                    Directory.CreateDirectory(serverPath);
                    model.Avatar.SaveAs(serverPath + filename + extension);

                    UpdateModel(editedUser.Informations);
                    ...
                    using (var db = new ApplicationDbContext())
                    {
                        ...
                        _dbUser.Informations = editedUser.Informations;
                        _dbUser.Informations.Avatar = "/Content/Upload/" + filename + extension;
                    ...
                    var body = EmailTemplate.generateEmail(bodyPartial, user.UserName);
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ...
            using (var db = new ApplicationDbContext())
            {
                var _dbUser = db.Users.Where(x => x.UserName == User.Identity.Name).FirstOrDefault();
                if (_dbUser.Informations != null)
                {
                    ViewBag.Avatar = _dbUser.Informations.Avatar;
                }
                else
                {
                    ViewBag.Avatar = "http://epaper2.mid-day.com/images/no_image_thumb.gif";
                }
